...
use crate::models::{EditPostInput, Post, WritePostInput};
...
fn require_auth() -> anyhow::Result<Credentials> {
    auth::load_credentials()?.ok_or_else(|| {
        anyhow::Error::new(AuthError)
            .context("Not logged in. Run `velog auth login` first.")
    })
}
...
pub async fn auth_login() -> anyhow::Result<()> {
    ...
    eprintln!(
        "Find them in browser DevTools → Application → Cookies → velog.io"
    );

    eprint!("access_token: ");
    let access_token =
        rpassword::read_password().context("Failed to read access_token")?;
    anyhow::ensure!(
    ...
    let refresh_token =
        rpassword::read_password().context("Failed to read refresh_token")?;
    anyhow::ensure!(
